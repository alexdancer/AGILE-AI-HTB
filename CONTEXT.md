...
**Properties**: Uses the Harness LiteLLM transport path so its spend is tracked as Orchestration Tokens labeled task breakdown. It performs semantic decomposition rather than relying on simple length thresholds. It uses lightweight project context such as language, framework, test command, top-level folders, and project docs rather than inspecting arbitrary source files. It follows tracer-bullet vertical-slice rules and returns either a single-task decision or proposed smaller Tasks for human review.
