...
## Scout Task
**Definition**: A Task whose deliverable is a report rather than a code change, used to investigate a question or de-risk a low-confidence estimate.
**Properties**: A Scout is an ordinary Orchestration Board Task with candidate kind `scout`. It is estimated, budgeted, launched through a Worker Adapter, and lands in Review like any other Task, so its spend appears as Task actuals on a visible card rather than as hidden orchestration overhead. It runs read-only: it may inspect files and configuration and run targeted non-mutating discovery commands, but must not modify code, run destructive commands or migrations, or commit. Its output is a session report artifact containing findings, risks, and a recommendation. It reuses the read-only launch path that is already the first verified OpenCode launch proof.
**Relationships**: Created by the User, or proposed by the Task Breakdown Agent as a candidate. Low-confidence Task Estimation does not silently trigger investigation spend; it raises a Needs You entry offering to launch a Scout, accept the estimate, or estimate manually. A Scout's findings inform a re-estimate of the implementation Task, which the User applies; the Harness does not automatically rewrite an estimate the User has already seen. Replaces the former Spike concept, which modeled investigation as a hidden action on a Task rather than as governed, visible work.
...
**Relationships**: Produced by the Task Breakdown Agent, Task Estimation, Worker Adapter verification, Agent Review, and reporting. Persisted token usage is labeled by usage kind such as worker, task breakdown, estimation, adapter verification, or reporting with spend-category metadata when needed. Task Breakdown Agent tokens are labeled as task breakdown, not attached to Task implementation actuals, and shown as orchestration spend. Scout Tasks are deliberately not Orchestration Tokens: a Scout is a real Task launched through a Worker Adapter, so its spend is Worker spend recorded against that Task's actuals. Worker Adapter verification tokens are labeled as adapter verification, not attached to Task actuals, and hidden from the normal task board or marked as system verification. Agent Review/reporting tokens use the control-plane model and remain separate from Worker execution spend. Shown separately from Worker tokens in Portal analytics while still counting toward the daily budget.
...
## Needs You
**Definition**: The project-scoped queue of everything awaiting a human decision before work in that project can continue.
**Properties**: Scoped to one Connected Project, matching the rest of the Orchestration Board, so a decision is always read in the context of the project's repository, adapter, and budget. Aggregates pending Proposed Task Breakdowns awaiting review, Tasks needing a manual estimate, launches refused by Launch Guardrails, completed Worker Runs awaiting Review Disposition, and budget overrides awaiting approval. Each entry names its reason and the action that clears it. It reflects the escalate-only supervision model: the Harness proceeds autonomously and surfaces only decisions it must not make alone.
**Relationships**: Distinct from Alarms. Needs You is about decisions blocking forward progress; Alarms are runtime behavioral warnings about a Worker already running, such as budget burn, loop detection, and tool-category bias. They must not be merged. Draws from Blocked Condition, Proposed Task Breakdown, Task Estimation, Launch Guardrails, and Review Disposition.
...
**Properties**: The Evidence Drawer exposes Agent Review, Mark Done, and Block actions when completed Worker Run or Session evidence is present. The operator may save a review prompt or focus while leaving the Task in Review. Mark Done records an accepted operator decision and moves the Task to Done. Block requires a human-readable reason, records blocked review metadata as a Blocked Condition, and keeps the Task in Review. Review evidence remains linked to the Task regardless of disposition.
