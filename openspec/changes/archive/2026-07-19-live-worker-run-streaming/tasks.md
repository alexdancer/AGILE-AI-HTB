...
- [x] 1.1 Add `src/foreman_ai_hq/stream_events.py`: `streaming_runner(plan, on_event)` using
...
- [x] 1.2 Add `map_stream_event(self, raw_line) -> CommonEvent | None` to `WorkerAdapterBuilder`
...
- [x] 1.3 In `task_launch._run_worker_adapter` / `_execute_worker_run`, run the launch through
...
- [x] 2.1 Add `since_id: int | None` to `list_worker_run_events` (`:1128`): order by `id` for a
  lossless ID cursor (project `created_at`); when provided, return only events with `id > since_id`.

## 3. Events endpoint — `src/foreman_ai_hq/routes/sessions.py`

- [x] 3.1 Add authenticated `GET /api/sessions/{id}/events?since_id=` returning a bounded projection
...
- [x] 4.1 In the run timeline (Board.jsx project workspace + the timeline used by SessionReport.jsx),
...
- [x] 4.2 Render the common feed (`agent_message` / `tool_call` / `token` / `status`) as read-only
...
- [x] 5.1 Unit: golden fixtures of representative stream lines per adapter → `map_stream_event` yields
  the expected normalized events; secret-like content and the launch prompt are redacted.
- [x] 5.2 Unit: `streaming_runner` buffers stdout identical to `subprocess_runner` for the same
...
- [x] 5.3 Integration: launch with an injected fake streaming runner → `worker_run_events` rows appear
...
- [x] 5.4 Non-regression gate: `governance-integration-smoke` and `estimation-accuracy-tracking`
  suites stay green.
- [x] 5.5 Frontend: while Running, the feed appends streamed events without a manual refresh and
  settles on completion; live usage shows a provisional label; events render as evidence.
- [x] 5.6 Portal E2E (synthetic): extend the Recorded Demo Run to a governed launch backed by
...
- [x] 5.7 Gates: `uv run pytest -q`, `npm --prefix frontend run check`,
