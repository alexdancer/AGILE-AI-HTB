...
- **AND** the dashboard SHALL make clear that failed/retry attempt spend can make total Worker evidence exceed the number shown beside reviewable completed tasks
...
**Reason**: The product decision changed: cache-read/reused-context tokens make operator budget and task-actual comparisons misleading when they are treated as budget burn.
**Migration**: Use `Budget enforcement excludes cache reads`; keep cache-read tokens and provider raw totals as audit evidence instead of budget-used totals.
