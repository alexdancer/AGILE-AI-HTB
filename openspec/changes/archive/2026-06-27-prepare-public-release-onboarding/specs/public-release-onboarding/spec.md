...
The system SHALL provide a public README onboarding path that gets a first-time local operator from fresh clone to a tiny governed launch proof without requiring architecture-doc exploration.

#### Scenario: Operator follows first-run path
- **WHEN** a public operator reads the README quickstart
- **THEN** the documented happy path SHALL include install, `htb init`, `htb serve`, portal login, `/settings/control-plane` provider/model/API-key entry, explicit control-plane connection test, project connection, Worker Adapter setup, and a tiny launch proof
