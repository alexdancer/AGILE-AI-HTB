...
The system SHALL launch OpenCode Worker Sessions through a non-interactive command that includes the `run` subcommand, the selected Worker model, JSON output mode, and the scoped task prompt.

#### Scenario: OpenCode launch command includes model and prompt
...
- **AND** the system either normalizes it to the supported non-interactive run command or blocks launch with a clear compatibility reason
...
- **AND** the task metadata preserves sanitized return code, stdout, stderr, selected adapter, selected model, and command plan
