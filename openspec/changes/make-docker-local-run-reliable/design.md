...
4. Smoke script over test framework machinery.
   - Chosen: one runnable smoke path that detects `docker-compose` before `docker compose`, starts the service, curls endpoints, runs `htb seed-demo`, checks DB file, recreates the service to prove named-volume persistence, then tears down.
