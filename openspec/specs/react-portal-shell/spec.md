...
The React Portal shell SHALL own its dashboard home, selected project workspace, project board workflow, Sessions list, Session Report, Task Breakdown Review, and Project Task History while existing Jinja pages remain available for non-migrated workflows and as build-aware fallback for migrated surfaces. The selected React project workspace SHALL preserve the existing project overview's identity, profile, readiness, actionable summary, archive safety, and workflow navigation. The canonical `/sessions`, `/sessions/{session_id}`, `/task-breakdowns/{breakdown_id}/review`, and `/projects/{project_id}/task-history` routes SHALL select React only when the complete frontend build is available.
...
- **AND** this change SHALL NOT add `/app/sessions`, `/app/sessions/{session_id}`, `/app/task-breakdowns/{breakdown_id}/review`, or `/app/projects/{project_id}/task-history` aliases
