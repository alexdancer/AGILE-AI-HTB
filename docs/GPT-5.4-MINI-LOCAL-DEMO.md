...
## 2. Initialize local operator setup

```bash
uv run htb init
```

`htb init` writes `.htb/config.toml` with non-secret local defaults and `.htb/secrets.env` with local-only secrets. `.htb/` is ignored by git. Demo seeding is a separate optional step below.

### Set the provider key once

Edit `.htb/secrets.env` and replace the placeholder value. Keep the generated portal token or replace it with your own login token.

```bash
$EDITOR .htb/secrets.env
```
...
## 3. Start the portal from operator config
...
For the curl examples below in the same terminal, load the generated local secrets into the shell:
