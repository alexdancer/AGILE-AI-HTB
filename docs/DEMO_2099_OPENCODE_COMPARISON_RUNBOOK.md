...
## 3. Start AGILE-AI-HTB Locally

Use the project-local runner and a fresh demo database for this comparison.

```bash
export TOKEN_TRACKER_DATABASE_PATH="$PWD/.demo/opencode-comparison/harness.db"
export TOKEN_TRACKER_PORTAL_TOKEN="demo-token"
export AGILE_AI_HTB_CONTROL_MODEL="gpt-5.4-mini"

# Required: this is the portal/control-plane estimator key, not the OpenCode Worker auth.
printf 'Portal control-plane API key: '
stty -echo; read AGILE_AI_HTB_CONTROL_API_KEY; stty echo; printf '\n'
...
If the control-plane test says `missing control-plane API key env: AGILE_AI_HTB_CONTROL_API_KEY`, stop and restart the portal with the exports above before uploading the markdown task. Otherwise the board will create a `Blocked` task with `Estimator unavailable or invalid; manual estimate required.`
...
- configured workdir evidence showing files in `.demo/opencode-comparison/harness-target`;
